...
import com.tvshowtracker.exception.InvalidLoginException;
	...
	public static void run() {
		...
		while (true) {
			...
			while (!choice.matches("^[1-3]$")) {
				System.out.println(ConsoleColors.RED + "Not a valid choice.\n" + ConsoleColors.RESET);
				System.out.print(ConsoleColors.ITALIC + "Choose an option (1-3): "+ ConsoleColors.RESET);
			...
			if (choice.equals("1")) {
				...
				while(true) {
					...
					if (TVTrackerDaoSql.getUser(username,password) != null) 
						System.out.println(ConsoleColors.RED + "Username already taken.\n" + ConsoleColors.RESET);
		...
		while (true) {
			...
			if (option.equals("1")) {
				...
				while (!episodes.matches("^\\d+$")) {
					System.out.println(ConsoleColors.RED + "Not valid numeric input.\n" + ConsoleColors.RESET);
					System.out.print(ConsoleColors.ITALIC + "How many episodes does it have?: " + ConsoleColors.RESET);
				...
				while (!showIds.contains(idChoice)) {
					...
					System.out.println(ConsoleColors.RED + "That's not one of the available ids.\n" + ConsoleColors.RESET);
					System.out.print(ConsoleColors.ITALIC + "Enter the id of the show you want to delete: " + ConsoleColors.RESET);
				...
				if (op.equals("1")) {
					...
					while (!amount.matches("^\\d+$")) {
						System.out.println(ConsoleColors.RED + "Not a valid numeric input.\n" + ConsoleColors.RESET);
						System.out.print(ConsoleColors.ITALIC + "How many episodes for this show are able to be watched now: " + ConsoleColors.RESET);
	...
	public static void userSession(User user) {
		...
		while(true) {
			...
			try {
				...
				System.out.println(ConsoleColors.RED + "Not a valid option.\n" + ConsoleColors.RESET);
				option = 0;
				continue;
			}
			System.out.println();
			...
			{
				if(user.getList().isEmpty()) {
					System.out.println(ConsoleColors.RED + "No shows to update.\n" + ConsoleColors.RESET);
					break;
				}
				updateShow();
				break;
			...
			{
				if(user.getList().isEmpty()) {
					System.out.println(ConsoleColors.RED + "No shows to rate.\n" + ConsoleColors.RESET);
					break;
				}
				rateShow();
				break;
	...
	public static void rateShow() {
		...
		do {
			try {
				...
				System.out.println(ConsoleColors.RED + "Enter a number (1-5).\n"+ ConsoleColors.RESET);
				System.out.print(ConsoleColors.ITALIC + "What rating do you give the show (1-5): "+ ConsoleColors.RESET);
				goodInput = false;
			}
			catch(Exception e) {
